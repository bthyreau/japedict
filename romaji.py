#!/usr/bin/python
# Note:
# This code below is verbatim from http://halley.cc/code/?python/romaji.py
# provided by Ed Halley under the Artistic License
...
_chiisaiya = 'KGHBPR' # ki+ya, gi+ya, hi+ya, ...
_punctuals = { '-': u'\u30fc',
               '~': u'\u301c',
               '.': u'\u3002',
               ' ': '',
...
def roma(kana):
    ...
    roma = re.sub(r'XTSU([KGTDBP])', r'\1\1', roma)
    roma = re.sub(r'xtsu([kgtdbp])', r'\1\1', roma)
...
def kana(roma):
    ...
    '''
    roma = re.sub(r'([KGTDBP])\1', r'XTSU\1', roma)
    roma = re.sub(r'([kgtdbp])\1', r'xtsu\1', roma)
